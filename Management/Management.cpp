// Management.cpp : ´ËÎÄ¼þ°üº¬ "main" º¯Êý¡£³ÌÐòÖ´ÐÐ½«ÔÚ´Ë´¦¿ªÊ¼²¢½áÊø¡£
//
...
int configCQ()
{
	...
	cout << "password:\n";
	cin >> password;
	cout << "Ê¹ÓÃµÄQSign\n(ÊäÈë-½ûÓÃQSign·þÎñÆ÷)\n(ÍÆ¼öÊäÈë0Ê¹ÓÃ±¾µØQSign·þÎñÆ÷):\n";
	cin>>QSign;
	cout << "access-token:\n(Ç¿ÁÒ½¨ÒéÅäÖÃÔÚ¹«ÍøµÄ·þÎñÆ÷ÉèÖÃaccess-token,Èç¹ûÊ¹ÓÃ±¾µØgo-cqhttp,»á×Ô¶¯Í¬²½access-token)\n";
	cin >> accessToken;
...
public:
	...
	void on_message(websocketpp::connection_hdl, client::message_ptr msg) {
		if (msg->get_opcode() == websocketpp::frame::opcode::text) {
			...
			string post_type;
			string message;
			try { message = jm["message"]; }//messageÎªÏûÏ¢ÄÚÈÝ£¬Îªstring
			catch (...) {}
			try { role = jm["sender"]["role"]; }//roleÎª·¢ËÍÕßµÄÈºÁÄÉí·Ý£¬¿ÉÑ¡Öµ£º"owner"ÈºÖ÷   "admin"¹ÜÀíÔ±   "member"³ÉÔ±,±äÁ¿ÀàÐÍÎªstring
			catch (...) {}
			try { userid = to_string(jm["user_id"]); }//useridÎª·¢ËÍÕßQQºÅ£¬Îªstring
			catch (...) {}
			try
			{
			...
			{
				if (message == GBK_2_UTF8("¿ª·þ") && OpCheck(userid, role) == true)
				{
					cout << "ÕýÔÚ¿ª·þ" << endl;
...
int main()
{
	...
	if (info["manager"]["multi_server"] == false)
	{
		if (aleadyConfig == false)
		{
			...
			cout << "ÅäÖÃ»úÆ÷ÈË»ù´¡ÐÅÏ¢:" << endl << "ÄãµÄ·þÎñÆ÷ÈºQQºÅ:" << endl;
			cin >> GROUPIDINT;
			cout << "ÇëÊäÈëÄãµÄ·þÎñÆ÷Ãû³Æ" << endl;
			cin >> serverName;
		...
		if ((QSign.find("127.0.0.1") != QSign.npos) || (QSign.find("0.0.0.0") != QSign.npos))
		{
			...
			cout << "¼ì²âµ½QSignÆô¶¯£¬ÕýÔÚÆô¶¯go-cqhttp" << endl;
		}
		Sleep(3000);
		thread tl(startCq);
		tl.detach();
		if ((cq_ip.find("127.0.0.1") != cq_ip.npos) || (cq_ip.find("0.0.0.0") != cq_ip.npos))
		{
			...
			cout << "¼ì²âµ½go-cqhttpÆô¶¯,Æô¶¯Âß¼­µ¥Ôª" << endl;
		}
		Sleep(1000);
	}
